"""
Session 3, Block 2: TechFlow Support Agent — STARTER (Fill in the blanks!)

A simple agent that demonstrates the agentic loop pattern.
It has 3 tools: lookup_customer, check_order_status, create_ticket

YOUR JOB: Complete the 6 TODOs below to build the agentic loop.
The mock data, tool definitions, and tool executor are done for you.

Run with: python agent.py
Custom query: python agent.py "What's the status of order ORD-5678?"
"""
...
# ============================================================
# THE AGENTIC LOOP — YOUR JOB: Complete the TODOs!
# ============================================================
...
def run_agent(user_message: str) -> str:
    ...
    # ----------------------------------------------------------
    # HINT: response = client.messages.create(model=..., max_tokens=..., system=..., tools=..., messages=...)
    ...
    # ----------------------------------------------------------
    # TODO 2: Write the while loop condition
    # The loop should continue as long as:
    #   - response.stop_reason == "tool_use"  AND
    #   - loop_count < max_loops
    # ----------------------------------------------------------
    # HINT: while response.stop_reason == "..." and loop_count < ...:
    ...
    while response.stop_reason == "tool_use" and loop_count < max_loops:
        ...
        # Filter for blocks where block.type == "tool_use"
        # ----------------------------------------------------------
        # HINT: tool_blocks = [block for block in response.content if block.type == "..."]
        ...
        #   - Call execute_tool(block.name, block.input) to get the result
        #   - Build a tool_result dict with:
        #       "type": "tool_result"
        #       "tool_use_id": block.id
        #       "content": result
        # Collect all tool_results in a list
        # ----------------------------------------------------------
        # HINT: tool_results = []
        ...
        # Append TWO messages:
        #   1. The assistant's response: {"role": "assistant", "content": response.content}
        #   2. The tool results:         {"role": "user", "content": tool_results}
        # ----------------------------------------------------------
        # HINT: messages.append({"role": "assistant", "content": ...})
        ...
        # ----------------------------------------------------------
        # TODO 6: Make the next API call (same as TODO 1)
        # Call client.messages.create() again with the updated messages
        # Store the result back in `response`
        # ----------------------------------------------------------
